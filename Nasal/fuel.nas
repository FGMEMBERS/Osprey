...
io.include("Aircraft/ExpansionPack/Nasal/fuel.nas");
...
setlistener("/sim/signals/fdm-initialized", func {
    ...
    var left_engine_flow = func (flow) {
        var lbs_hour = get_gain(getprop("/rotors/tail/rpm")) * default_lbs_hour / 2;
        setprop("/v22/pfcs/internal/left-lbs-hour", lbs_hour);
    ...
    var right_engine_flow = func (flow) {
        var lbs_hour = get_gain(getprop("/rotors/main/rpm")) * default_lbs_hour / 2;
        setprop("/v22/pfcs/internal/right-lbs-hour", lbs_hour);
    ...
    var tank_left_wing_feed  = Tank.new("left-wing-feeder", 2);
    var tank_right_wing_feed = Tank.new("right-wing-feeder", 3);

    # Left/Right Engines
    var left_engine  = EngineConsumer.new("left", left_engine_flow);
    var right_engine = EngineConsumer.new("right", right_engine_flow);

    # Pumps for the two engines
    var pump_left_feed_engine  = AutoPump.new("left-feed-engine", 1.0);
    var pump_right_feed_engine = AutoPump.new("right-feed-engine", 1.0);
    ...
    var manifold_feeders = Manifold.new("feeders");

    # Sinks attached to the manifold
    var tube_left_feeder  = Tube.new("left-feeder", default_max_capacity);
    var tube_right_feeder = Tube.new("right-feeder", default_max_capacity);

    # Sources attached to the manifold
    var pump_left_fwd_sponson  = BoostPump.new("left-fwd-sponson", default_max_capacity);
    var pump_right_fwd_sponson = BoostPump.new("right-fwd-sponson", default_max_capacity);
    var pump_right_aft_sponson = BoostPump.new("right-aft-sponson", default_max_capacity);
    ...
    # The sponson tanks
    var tank_left_forward_sponson  = Tank.new("left-forward-sponson", 0);
    var tank_right_forward_sponson = Tank.new("right-forward-sponson", 1);
    var tank_right_aft_sponson     = Tank.new("right-aft-sponson", 4);
    ...
    # Sources attached to the manifold
    var pump_fwd_mats_one   = BoostPump.new("fwd-mats-1", default_max_capacity);
    var pump_fwd_mats_two   = BoostPump.new("fwd-mats-2", default_max_capacity);
    var pump_aft_mats_three = BoostPump.new("aft-mats-3", default_max_capacity);
    ...
    # The MATS tanks
    var tank_fwd_mats_one   = Tank.new("fwd-mats-1", 5);
    var tank_fwd_mats_two   = Tank.new("fwd-mats-2", 6);
    var tank_aft_mats_three = Tank.new("aft-mats-3", 7);
    ...
    # Sources attached to the manifold
    var pump_left_wing_aux  = BoostPump.new("left-wing-aux", default_max_capacity);
    var pump_right_wing_aux = BoostPump.new("right-wing-aux", default_max_capacity);
    ...
    var tank_left_wing_aux  = Tank.new("left-wing-aux", 8);
    var tank_right_wing_aux = Tank.new("right-wing-aux", 9);
    ...
    var update_state = func {
        manifold_feeders.prepare_distribution();

        pump_left_feed_engine.transfer_fuel();
        pump_right_feed_engine.transfer_fuel();

        pump_right_aft_sponson.transfer_fuel();

        pump_aft_mats_three.transfer_fuel();

        pump_fwd_mats_one.transfer_fuel();
        pump_fwd_mats_two.transfer_fuel();

        pump_left_wing_aux.transfer_fuel();
        pump_right_wing_aux.transfer_fuel();

        pump_left_fwd_sponson.transfer_fuel();
        pump_right_fwd_sponson.transfer_fuel();
    };

    var state_timer = maketimer(1 / frequency, update_state);

    # Pause updating fuel state if simulation is paused
